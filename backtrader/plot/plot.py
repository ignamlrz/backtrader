...
class Plot_OldSync(with_metaclass(MetaParams, object)):
    ...
    def newaxis(self, obj, rowspan):
        ax = self.mpyplot.subplot2grid((self.pinf.nrows, 1), (self.pinf.row, 0), rowspan=rowspan, sharex=self.pinf.sharex)
    ...
    def plotind(self, iref, ind, subinds=None, upinds=None, downinds=None, masterax=None):
        ...
        for lineidx in range(ind.size()):
            ...
            if self.pinf.sch.valuetags and vtags:
                ...
                if linetag and not math.isnan(lplot[-1]):
                    ...
                    self.drawtag(ax, len(self.pinf.xreal), lplot[-1], facecolor=self.pinf.sch.locbgother, edgecolor=self.pinf.color(ax))
            ...
            for fcmp, fop in farts:
                ...
                if fref is not None:
                    ...
                    ax.fill_between(self.pinf.xdata, y1, y2, facecolor=fcol, alpha=falpha, interpolate=True, **kwargs)
        ...
        if not masterax:
            ...
            for hline in hlines:
                ax.axhline(hline, color=self.pinf.sch.hlinescolor, ls=self.pinf.sch.hlinesstyle, lw=self.pinf.sch.hlineswidth)
            ...
            if self.pinf.sch.legendind and ind.plotinfo._get("plotlegend", True):
                ...
                if labels:
                    ...
                    legend = ax.legend(loc=loc, numpoints=1, frameon=False, shadow=False, fancybox=False, prop=self.pinf.prop)
        ...
        for bname in getattr(ind, "plotboxes", []):
            boxes = getattr(ind, bname)
            for box in boxes:
                color = getattr(getattr(ind, "p"), bname + "_color")
                alpha = getattr(getattr(ind, "p"), bname + "_alpha", 0.3)
                x0, x1, y0, y1, highlight = box
                if self.pinf.xend < x0 or x1 < self.pinf.xstart:
                    continue
                x0 = max(x0 - self.pinf.xstart - 1, 0)
                x1 = min(x1 - self.pinf.xstart, self.pinf.xend - self.pinf.xstart)
                rect = patches.Rectangle((x0, y0), width=x1 - x0, height=y1 - y0, color=color, alpha=alpha)
                ax.add_patch(rect)
    ...
    def plotdata(self, data, indicators):
        for ind in indicators:
            ...
            for upind in upinds:
                self.plotind(data, upind, subinds=self.dplotsover[upind], upinds=self.dplotsup[upind], downinds=self.dplotsdown[upind])
        ...
        if self.pinf.sch.style.startswith("line"):
            ...
            if self.pinf.sch.linevalues and plinevalues:
                datalabel += " O:%.2f H:%.2f L:%.2f C:%.2f" % (opens[-1], highs[-1], lows[-1], closes[-1])
        ...
        if self.pinf.sch.valuetags and vtags:
            self.drawtag(ax, len(self.pinf.xreal), closes[-1], facecolor=self.pinf.sch.locbg, edgecolor=self.pinf.sch.loc)
        ...
        if handles:
            ...
            legend = axlegend.legend(h, l, loc=loc, frameon=False, shadow=False, fancybox=False, prop=self.pinf.prop, numpoints=1, ncol=1)
        ...
        for ind in indicators:
            ...
            for downind in downinds:
                self.plotind(
                    data, downind, subinds=self.dplotsover[downind], upinds=self.dplotsup[downind], downinds=self.dplotsdown[downind]
                )
